...
async def _async_mp_client_check_health_async(self) -> None:
    ...
    try:
        ...
        logger.error(
            "EngineCore did not respond to health ping within %ss "
            "- marking engine as dead", timeout)
...
async def _dp_lb_client_check_health_async(self) -> None:
    ...
    logger.debug(
        "Sending health ping to %d EngineCores (timeout=%ss)",
        num_engines, timeout)
    try:
        results = await asyncio.wait_for(
            asyncio.gather(
                *[
                    self._call_utility_async("health_ping", engine=engine)
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                self.resources.engine_dead = True
                raise EngineDeadError(
                    f"Engine {i} health ping failed: {result}")
        logger.debug("Health ping succeeded for all %d engines", num_engines)
    except asyncio.TimeoutError:
        logger.error(
            "One or more EngineCores did not respond to health ping "
            "within %ss - marking engine as dead", timeout)
        self.resources.engine_dead = True
        raise EngineDeadError(
            f"One or more EngineCores did not respond to health ping "
            f"within {timeout}s")
    except EngineDeadError:
        raise
    except Exception:
...
core_client_module.AsyncMPClient.check_health_async = (
    _async_mp_client_check_health_async)
core_client_module.DPLBAsyncMPClient.check_health_async = (
...
logger.info(
    "patch_health_check: applied health ping (timeout=%ss)",
